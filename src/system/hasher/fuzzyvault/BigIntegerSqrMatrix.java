...
/**
 * BigIntegerSqrMatrix class is used for manipulating BigInteger Squere 
 * Matrix. provides the following amin features: inversion, turning a 
 * matrix to upper diagonal form, solving homogenious equations system 
 * based on the matrix.
 */
public class BigIntegerSqrMatrix {
	...
	public static void main(String[] args) throws NoSuchAlgorithmException {
		...
		BigIntegerSqrMatrix mat = new BigIntegerSqrMatrix(A,BigInteger.valueOf(7919));
		...
		System.out.println("test by multiplying the matrix with a private solution vector");
	...
	/**
	 * c'tor, creates the matrix instance from a BigInteger 2D - array
	 * All calculations will be performed in Zp where p == modulo
	 * @param mat a 2D array containing the values of the matrix, should be 
	 * 		of the same dimensions (nXn)
	 * @param modulo the modulo of the field. all calculations will be performed
	 * 		with mod(modulo)
	 * @throws MatrixDimensionsException if mat dimensions are
	 * not equal
	 */
	 ...
	 * @param modulo the modulo of the field on which the calculations 
	 * 		are performed
	 */
	 ...
	 * @param izeX the X dimensions of the area to copy 
	 * @param y- y dimesion skew between source and destination
	 * @param x- x dimesion skew between source and destination
	 */
	private static void copyArray(BigInteger[][] target, BigInteger[][] source, int sizeY, int sizeX, int y, int x) {
	 ...
	 * @throws MatrixNotInvertiableException if the matrix is not invertiable
	 * @param
	 * 	D the array to be inverted
	 *  n the diemnsion to be inverted (assuming D is of dimensions nX2n)
	 *  fieldModulo the modulo of the field on which the calculation are 
	 * 		performed
	 */
	private static BigInteger[][] invertArray(BigInteger[][] D, int n,BigInteger fieldModulo) {
		BigInteger alpha;
		BigInteger beta;
		int i;
		int j;
		...
		for (i = 0; i < n; i++) {
			...
			if (alpha.equals(BigInteger.ZERO)) /* error - singular matrix */ {
				...
				for (j = 0; j < n2; j++) {
					D[i][j] = D[i][j].multiply(alpha.modInverse(fieldModulo)).mod(fieldModulo);
				}
				...
				for (k = 0; k < n; k++) {
					if ((k - i) != 0) {
						beta = D[k][i];
						for (j = 0; j < n2; j++) {
							D[k][j] = D[k][j].subtract(beta.multiply(D[i][j]).mod(fieldModulo)).mod(fieldModulo);
						}
					}
				}
			}
	 ...
	 * @param 
	 * 	D 2D array to be turned into diagonal form
	 * 	n the dimension of the array
	 * 	fieldModulo	 the modulo of the field used for BigInteger 
	 * 		calculation
	 */
	private static BigInteger[][] diagonalArray(BigInteger[][] D, int n,BigInteger fieldModulo) {
		BigInteger alpha;
		BigInteger beta;
		int i;
		int j;
	...
	public BigIntegerSqrMatrix multiply(BigIntegerSqrMatrix mat) {
		BigIntegerSqrMatrix res = new BigIntegerSqrMatrix(this.n,this.fieldModulo);
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				res.values[i][j] = BigInteger.ZERO;
				for (int k = 0; k < n; k++)
					res.values[i][j] = res.values[i][j].add(this.values[i][k].multiply(mat.values[k][j])).mod(fieldModulo);
			}
		}
		return res;
	}
	...
	public BigInteger[] multiply(BigInteger[] vector) {
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = 0; j < n; j++) {
				res[i] = res[i].add(this.values[i][j].multiply(vector[j])).mod(fieldModulo);
			}
		}
		return res;
	}
