...
public class PCA extends Quantizer {
	...
	public PCA() {
		...
		this.numberOfComponents = PCASettings.getInstance()
				.numberOfComponents().getValue();
	}
	...
	public void train(Users trainingUsers) {
		...
		for (int j = 0; j < this.numberOfComponents; j++) {
			...
			this.variableSettings.get(j).setBins(
					PCASettings.getInstance().bitsPerComponent().getValue());
	...
	public class PCAFeature extends Feature {
		...
		public PCAFeature() {
			...
			methodVar.setBins(PCASettings.getInstance().bitsPerComponent()
					.getValue());
			for (int i = 0; i < PCASettings.getInstance().numberOfComponents()
					.getValue(); i++) {
				this.variables
						.put("Component " + i, new PCAVariable(methodVar));
			}
		}
