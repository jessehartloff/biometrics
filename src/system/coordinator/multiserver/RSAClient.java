...
public class RSAClient {
	...
	public void RSAKeyExchange() throws NoSuchAlgorithmException,
		...
		System.out.println("Took " + keyGenTime
				+ " milliseconds to generate key pair ");

		System.out.println("Public key is " + publicKeyString);
		// System.out.println("Length of public key is :" +
		...
		ObjectOutputStream objOut = new ObjectOutputStream(
				clnt_socket.getOutputStream());
		objOut.writeObject(publicKey);

		/*
		 * byte[] inputBuffer = new byte[128]; BufferedInputStream inputStream =
		 * new BufferedInputStream(clnt_socket.getInputStream());
		 * inputStream.read(inputBuffer,0,128);
		 * System.out.println("Received Encrypted Key :" +new
		 * String(inputBuffer, "UTF-8"));
		 * 
		 * long startDecryption = System.currentTimeMillis(); Cipher cipher =
		 * Cipher.getInstance("RSA"); cipher.init(Cipher.DECRYPT_MODE,
		 * privateKey); byte[] decryptedKey = cipher.doFinal(inputBuffer);
		 * System.out.println("Decrypted Key :" +new String(decryptedKey,
		 * "UTF-8"));
		 * 
		 * long finishTime = System.currentTimeMillis();
		 * System.out.println("Time for Decryption is :" + (finishTime -
		 * startDecryption));
		 * System.out.println("Time For RSA Key Exchange Was : " +
	...
	public Cipher getCipherInstance(String cipherType) {
		...
		try {
			...
			System.out
					.println("Something is broke: could not get Cipher Instance");
		}
		return null;
	}

	public BigInteger decryption(Cipher cipher, PrivateKey privateKey,
			BigInteger minutia) {
		byte[] decryptedKey = null;
		try {
			// cipher = Cipher.getInstance("RSA");
			cipher.init(Cipher.DECRYPT_MODE, privateKey); // make sure to pull
															// this out before
															// doing multiple
															// decryptions when
															// you integrate
		...
		// if we need to send it can use an Object Output Stream : return
		// (Object)decryptedKey;
		return new BigInteger(decryptedKey);
	}

	public String decryptionString(Cipher cipher, PrivateKey privateKey,
			byte[] minutia) throws UnsupportedEncodingException {
		byte[] decryptedKey = null;
		try {
			// cipher = Cipher.getInstance("RSA");
			cipher.init(Cipher.DECRYPT_MODE, privateKey); // make sure to pull
	...
	public void testMethods() throws NoSuchAlgorithmException {
		...
		try {
			decryptedPassword = decryptionString(cipher, privateKey,
					encryptString(publicKey, password));
