...
public class Server1 extends system.coordinator.Coordinator {
	...
	public RawScores run() {
		//sit in infinite loop for receiving shit
		//while True: receive()
		return null;
	}
	...
	public void receive(){
		//make an ID class to handle uuid per transaction, server2 and client origin and user id, plus enrollment/testing boolean
		//0.) if it's the first time S1 has seen the user id, then it must be an enrollment 
		
		
		//1.) Receive input from client and Server_2
		//2.) Determine where the the input is coming from based on UUID presence/matching in some external data structure
		//2a.) Making an external thread to handle this process
		//3.) if there's a match, then call other functions that handle each case( from client or serve 2); these functions then call send(..)
	}
	...
	public void test(){
		...
		//2.) Compare the test Templates and the enrolled Templates with Hasher.compareTemplates
		//
	}
