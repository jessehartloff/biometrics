...
public class IndexTesting extends Coordinator {
	...
	public RawScores run() {
		...
		for (User user : this.users.users) {
			hasher.addToIndexingStructure(user.readings.get(0), new Long(
					user.id), indexingStructure);
			numberEnrolled++;
			System.out.println("Building indexing structure: "
					+ (numberEnrolled * 100.0) / (total * 1.0) + "%");
		}
		...
		for (User user : this.users.users) {
			...
			for (int i = 1; i < n; i++) {
				scores.indexRankings.add(hasher.findIndexingRank(
						user.readings.get(i), new Long(user.id),
						indexingStructure, numberEnrolled));
			}
		}
