...
public class Feature {
	...
	// @Override
	// public Double distanceFrom(Feature compareFeature/*,Distance
	// distanceFunction*/) {
	// //return distanceFunction.distance(this,compareFeature);
	// Double result = 0.0;
	// for(String varKey : variables.keySet()){
	// result +=
	// Math.pow(this.variables.get(varKey).getQuantizedValue().doubleValue() -
	...
	public boolean equals(Object obj) {
		...
		// for(String varKey : variables.keySet()){
		// result &=
		// this.variables.get(varKey).getQuantizedValue().equals(otherFeature.variables.get(varKey).getQuantizedValue());
		// }
		// return result;
	}
	...
	public String toString() {
		...
		for (Entry<String, Variable> var : variables.entrySet()) {
			// output += var.getKey() + ": " +
			// var.getValue().quantizedValue.toString() +" ";
			output += var.getKey() + ": "
					+ var.getValue().getPrequantizedValue().toString() + " ";
		}
		return output;
	}
	...
	public boolean prequantizedEquals(Feature otherFeature) {
		...
		for (String varKey : variables.keySet()) {
			result &= this.variables
					...
					.equals(otherFeature.variables.get(varKey)
							.getPrequantizedValue());
		}
		return result;
	}
	...
	public String prequantizedToString() {
		...
		for (Entry<String, Variable> var : variables.entrySet()) {
			output += var.getKey() + ": "
					+ var.getValue().prequantizedValue.toString() + " \n";
		}
		output += "]";
		return output;
	}
	...
	public boolean equalsPrequantized(Feature otherFeature) {
		...
		for (String var : this.variables.keySet()) {
			if (Math.abs(this.variables.get(var).getPrequantizedValue()
					- otherFeature.variables.get(var).getPrequantizedValue()) > 0.00001) {
				return false;
			}
		}
	...
	// toReturn =
	// toReturn.shiftLeft(variableSettings.get(i).getBits().intValue());
	// Long quantizedValue =
	// variableSettings.get(i).findBin(feature.variables.get(varName).getPrequantizedValue().doubleValue());
