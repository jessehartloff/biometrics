...
public class SettingsMethodVariable extends SettingsVariable {
	...
	public Long getBins() {
		SettingsLong binsSettings = (SettingsLong) this.settingsVariables
				.get("bins");
	...
	public void setBins(Long value) {
		SettingsLong binsLong = (SettingsLong) this.settingsVariables
				.get("bins");
	...
	public void computeBinBoundaries(ArrayList<Double> prequantizedValues) {
		...
		for (int i = 1; i < this.getBins(); i++) {
			Double cutoff = (prequantizedValues
					.get((binSize.intValue() * i) - 1) + prequantizedValues
					.get(binSize.intValue() * i)) / 2.0;
