 ...
 *
 */
public class ParameterOptimizeNGONAR {
...
//	this.settingsVariables.put("Modality", AllModalitySettings.getInstance())
	...
	NgonAllRotationsSettings ngonar; //stands for Triplets Of Triangles All Rotations
	 ...
	 * Constructor
	 */
	public ParameterOptimizeNGONAR( String trainData, String testData, int bMin, int bMax, int kMin, int kMax, int nMin, int nMax) {
		//set datasets
		...
		method.manuallySetComboBox(NgonAllRotationsSettings.getInstance());
		ngonar = NgonAllRotationsSettings.getInstance();
...
//		this.settingsVariables.put("rotationStop", new SettingsDouble(50.0));
				...
				for( int x = binMin; x <=binMax; x++ ){
					for( int y = binMin; y <=binMax; y++ ){
						for ( int k=kMin; k <=kMax; k++ ){
							...
							ngonar.xBins().setValue(x);
							ngonar.yBins().setValue(y);
							ngonar.kClosestMinutia().setValue(k);
							...
							if (/*testCount % 10*/ 0 == 0) {
						        ...
						        try {								      
						            ...
						            String fileName = binMin+"_"+binMax+"_"+kMin+"_"+kMax+"_"+nMin+"_"+nMax+"_"+ "_ngonar_optimize.txt";
	...
	public static void main(String[] args){
		...
		ParameterOptimizeNGONAR test = new ParameterOptimizeNGONAR(trainDB, testDB, bMin, bMax, kMi, kMa, nMin, nMax);
        ...
        try {								      
            ...
            String fileName = "final_"+bMin+"_"+bMax+"_"+kMi+"_"+kMa+"_"+nMin+"_"+nMax+"_ngonar_optimize.txt";
            File newTextFile = new File(fileName);
